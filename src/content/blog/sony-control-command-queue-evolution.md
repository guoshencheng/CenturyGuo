...
faq:
  - q: 相机控制 App 第一张图等了多久？为什么？
    a: 3.5 秒。USB 相机接 Mac，Electron + koffi FFI + C++ bridge + libgphoto2，`gp_camera_set_single_config` 要和相机握几次手才能完成。3.5 秒阻塞期间 Electron 主线程被冻住，窗口不能拖、按钮不能点，连 loading 动画都不动了——用户视角是应用死了。这不是代码写得不好，是 FFI 同步阻塞本身的代价。
  - q: 第一次重构为什么把队列放在 C++ 里？
    a: v1 想在 bridge 层加异步队列解决阻塞。C++ 用 std::deque + std::condition_variable + 工作线程，bridge_enqueue_cmd 投递、bridge_poll_result 轮询。还加了优先级（TERMINATE/DISCONNECT 用 push_front 插队）和预览去重（PREVIEW_FRAME 不入队列）。但跑通后发现三个问题：poll 循环本身阻塞且 CPU 空转；C++ 多了 200 行线程代码，gdb 调试成本远高于 Chrome DevTools；新增命令要改 3 处（CmdType 枚举 + switch case + TS dispatch）。
  - q: 为什么 v2 反而把队列搬回 JS？
    a: 因为 Electron 已经有 worker_threads 了，Worker 线程本身就是"异步隔离壳"，在它之上再造一个隔离壳是过度设计。v2 让 Worker 一次只跑一个 C++ 调用（同步阻塞没关系，反正不是主线程），JS 层用两个 Map（pending 路由命令、streams 路由流回调）完成队列职责。新增命令只改一行 `register()`，调试栈回到 Chrome DevTools 而不是 gdb。
  - q: v2 比 v1 快多少？
    a: v2 把 3.5 秒的阻塞从"主线程"移到了"Worker 线程"，用户完全无感——Electron 主线程 50ms 内就能响应下一次 UI 操作。调度延迟从 10ms poll 降到 0（直调），C++ 线程代码从 200 行降到 0 行（只剩 std::timed_mutex），新增命令成本从 3 处降到 1 处。两条方法论：异步边界贴近"真正会卡死主线程的地方"，不是越靠底层越好；Worker 线程是天然的隔离壳，不要在它之上再造一层异步。
---
