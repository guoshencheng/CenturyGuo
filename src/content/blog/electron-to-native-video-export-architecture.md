...
faq:
  - q: 17 分钟视频导出要多久？为什么这么慢？
    a: 用 Electron 同源渲染方案要 8-15 分钟。瓶颈是 bitmap 数据流：每一帧从 GPU 显存 readback 到 CPU 内存变成 bitmap（一帧 1920×1080×4 = 8MB），再拷贝给 ffmpeg。一秒 30 帧 = 240MB/s 内存流量 + IPC 序列化 + ffmpeg 编码开销。这是架构决定的，不是代码能优化的——Electron 浏览器环境的渲染管线天然走 CPU + bitmap 路径。
  - q: 多窗口并发能加速导出吗？
    a: 不能，反而更慢。开 4 个隐藏 offscreen 窗口并行截图，理论上能压到 1/4 时间，但实测内存峰值飙到 4GB+、系统开始疯狂 swap、导出时间反而更长。bitmap 不是轻量数据，4 窗口 × 8MB/帧 = 32MB/帧的内存对象，ffmpeg 编码跟不上就在内存堆积成山。一周后回滚了并发方案。教训：bitmap 时代的并发拷贝成本远超 CPU 计算收益，多核并行解决不了数据形态的瓶颈。
  - q: 为什么必须放弃 Electron 重写？
    a: 三个数字决定的：(1) 单帧 5-15ms 是 CPU 渲染上限；(2) ~100ms 预览响应是 Electron 进程隔离的 IPC 开销；(3) ~2GB 内存峰值是多进程模型 + bitmap 数据流的必然结果。Electron 主进程和渲染进程之间天然 IPC 开销，浏览器渲染管线天然走 CPU + bitmap 路径，Node.js 内存管理天然不适合处理每秒几百兆的像素数据流。解法必须是 GPU 渲染 + 同进程零拷贝。
  - q: 原生重写后性能提升多少？
    a: 17min 1080p 导出 8-15 分钟 → < 5 分钟（3x 提升）；单帧渲染 5-15ms → 2-5ms（3x）；预览响应 ~100ms → < 50ms（2x）；内存峰值 ~2GB → < 1.5GB（-25%）。关键是零拷贝 GPU 纹理复用——Skia GPU 渲染 HUD → Metal FBO → IOSurface-backed texture，被编辑预览、时间轴拖动、视频导出三种场景共享，像素数据从未离开 GPU 显存。Clean Architecture 四层让 Core 层 70-80% 代码可复用到 iOS 版本。
---
